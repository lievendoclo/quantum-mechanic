...
export class BitbucketProjectAdded implements HandleEvent<any> {
    ...
    public handle(event: EventFired<any>, ctx: HandlerContext): Promise<HandlerResult> {
        ...
        return ctx.messageClient.addressChannels({
            text: `
A new Bitbucket project has been created successfully by @${addedEvent.createdBy.slackIdentity.screenName} 👍
Click here to view your new project in Bitbucket: ${addedEvent.bitbucketProject.url}`,
            attachments: [
                {
                    text: `
...
If you would like to associate more teams to the *${addedEvent.project.name}* project, please use the \`@atomist subatomic associate team\` command`,
    ...
    private docs(): string {
        return `${url("https://subatomic.bison.absa.co.za/docs/projects#bitbucket",
            "documentation")}`;
    }
}
